...
const pageInfo={
  ...
  approval:{title:'医务审核',eyebrow:'REFERRAL REVIEW',description:'来源医院复核转诊信息与交接材料。'},
...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled)return;
  ...
  if(b.dataset.preset){importedPatient=null;inputSource=null;attachmentController.clear();activePreset=b.dataset.preset;patient={...presets.find(p=>p.id===activePreset)};caseText=patient.text;filter='all';invalidateCase();renderWorkbench();return;}
