...
function updatePatient(){const sex=caseText.match(/(?:患者|性别)[，,：:\s]*(男|女)|(?:^|[，,\s])(男|女)(?:性|[，,\s])/);const age=caseText.match(/(\d{1,3})\s*岁/);patient={...patient,name:activePreset?presets.find(p=>p.id===activePreset).name:'待转诊患者',sex:sex?.[1]||sex?.[2]||null,age:age?Number(age[1]):null};}
...
function navigate(next){
  ...
  if(page!==next){if(page==='downward')downwardWorkspace?.onLeave?.();if(page==='guidance')guidanceWorkspace?.onLeave?.();}
...
function renderWorkbench(){
  $('#page-content').innerHTML=`<div class="steps"><span class="done"><i>1</i>录入病例</span><b></b><span class="current"><i>2</i>匹配接诊医生</span><b></b><span><i>3</i>确认转诊</span></div><div class="workspace"><div><section class="panel"><div class="panel-title"><h2>${icon('file')}转诊病例</h2><small>医生发起</small></div><div class="case-body"><label class="field-label">病例模板 <span class="example-label">· 快速填写</span></label><div class="example-tabs">${presets.map(p=>`<button data-preset="${p.id}" class="${activePreset===p.id?'active':''}">${p.label}</button>`).join('')}</div><div class="patient-block" id="patient-summary"></div><div class="input-group"><label class="field-label" for="source-hospital">来源医院</label><input id="source-hospital" maxlength="80" value="${esc(sourceHospital)}"></div><div class="input-group"><label class="field-label" for="source-doctor">发起医生</label><input id="source-doctor" maxlength="40" value="${esc(sourceDoctor)}"></div><div class="input-group"><label class="field-label" for="case-input">患者病情与转诊原因</label><textarea id="case-input" maxlength="5000" placeholder="请输入患者症状、检查结果、本院能力缺口及转诊目的…">${esc(caseText)}</textarea><div class="case-footer"><span>请填写脱敏病例摘要</span><span id="case-length">${caseText.length} / 5000</span></div></div><div id="service-notice" class="service-notice" role="status" hidden></div><button class="primary block" id="match-button">${icon('spark')}匹配接诊医生 ${icon('arrow')}</button></div></section><div class="tip-card">${icon('shield')}<span>提供检查结果和本院能力缺口，可以让匹配依据更清楚。最终转诊由医生复核、接收方确认。</span></div></div><div class="results-column"><section id="results" aria-live="polite" aria-busy="false"></section><section id="chat-panel" class="panel chat-panel" aria-label="转诊问答"></section></div></div>`;
  updatePatientSummary();renderProvider();renderResults();renderChat();
}
...
function renderResults(){
  ...
  const current=ranked.filter(d=>filter==='all'||(filter==='internal'&&d.department.includes('内科'))||(filter==='surgery'&&d.department.includes('外科'))||(filter==='rehab'&&d.department.includes('康复')));
...
function closeModal(){modalType=null;selectedDoctor=null;selectedDate=null;selectedSlot=null;currentRecord=null;$('#modal-root').innerHTML='';document.body.classList.remove('modal-open');if(restoreFocus?.isConnected)restoreFocus.focus();}
...
function renderDoctor(){if(!hasCurrentMatch()||!selectedDoctor){closeModal();toast('请先完成当前病例的匹配。');return;}const d=selectedDoctor;const schedule=buildSchedule(d.id);const day=schedule.find(s=>s.date===selectedDate);const urgent=analysis.urgent;
  const body=`<div class="detail-top">${photo(d,true)}<div><h2>${esc(d.name)}<span>${esc(d.title)}</span></h2><p>北京安贞医院安徽医院 · ${esc(d.department)}</p><div class="tags">${d.expertise.slice(0,4).map(t=>`<span class="tag">${esc(t)}</span>`).join('')}</div></div>${score(d)}</div><div class="detail-body"><div class="detail-section"><h3>医生简介</h3><p>${esc(d.bio)}</p>${d.education?`<p style="margin-top:10px">学历与经历：${esc(d.education)}</p>`:''}<div class="reason-panel"><h3>与本病例的匹配依据</h3>${d.reasons.map(r=>`<p class="match-reason">${icon('check')}<span>${esc(r)}</span></p>`).join('')}<div class="breakdown">${(d.breakdown||[]).map(b=>`<div class="breakdown-line"><span>${esc(b.label)}</span><span class="track"><i style="width:${Math.max(0,Math.min(100,b.value/b.max*100))}%"></i></span><b>${esc(b.value)}/${esc(b.max)}</b></div>`).join('')}</div><p>100 分制匹配分用于辅助复核专业方向，不是临床概率；仍需转诊医生判断。</p></div><details class="sources"><summary>查看资料来源</summary><p>资料更新于 2026 年 9 月 14 日，专业信息以院方最新发布为准。</p>${d.sourceUrls.slice(0,4).map((u,i)=>`<a href="${esc(u)}" target="_blank" rel="noopener noreferrer">公开资料 ${i+1} ↗</a>`).join('')}</details></div><div class="detail-section"><h3>${urgent?'紧急转诊安排':'选择意向日期与时段'}<span class="schedule-status">待院方确认</span></h3>${urgent?`<div class="urgent-callout"><strong>${icon('clock')}本病例需要立即对接</strong><p>进入紧急转诊流程，由接收方协调接诊团队、床位与转运。普通门诊排班不用于本次急症安排。</p></div><div class="immediate-slot">${icon('receive')}<div><strong>立即发起 · 转诊中心协调</strong><p>需联系接收方核实接诊团队、床位与转运安排。</p></div></div><details><summary class="subtle-link">查看普通门诊意向时段</summary>`:'<p>先选择意向日期，再选择时段。</p>'}<div class="date-grid">${schedule.map(s=>`<button class="date-btn ${s.date===selectedDate?'selected':''}" data-date="${esc(s.date)}" ${urgent||!s.slots.some(t=>t.remaining>0)?'disabled':''} aria-label="${esc(formatDate(s.date))}${s.slots.some(t=>t.remaining>0)?' 可选':' 暂不可选'}" aria-pressed="${s.date===selectedDate}"><small>${esc(s.weekday)}</small><b>${esc(s.date.slice(5).replace('-','/'))}</b><em>${s.slots.some(t=>t.remaining>0)?'可选':'暂不可选'}</em></button>`).join('')}</div>${day?`<div class="slot-grid">${day.slots.map(s=>`<button class="slot-btn ${s.id===selectedSlot?.id?'selected':''}" data-slot="${esc(s.id)}" ${s.remaining<=0?'disabled':''} aria-pressed="${s.id===selectedSlot?.id}"><strong>${esc(s.session)}</strong><p>${esc(s.time)}</p><small>${s.remaining>0?'可登记意向':'暂不可选'}</small></button>`).join('')}</div>`:!urgent?'<div class="empty" style="padding:30px 15px"><p>选择上方日期，查看上午与下午时段</p></div>':''}<p class="schedule-note">尚未同步院方排班；所选时段仅登记意向，以院方确认为准。</p>${urgent?'</details>':''}${renderWorkingSchedule(d.id)}</div></div><div class="modal-footer"><p>${urgent?'<b>紧急转诊 · 立即对接</b><br>提交后仍需接收方确认':selectedSlot?`<b>${esc(formatDate(selectedDate))} ${esc(selectedSlot.session)}</b><br>${esc(selectedSlot.time)} · 接诊意向`:'<b>请选择意向日期与时段</b><br>转诊资料会在下一步供你复核'}</p><button class="primary" data-action="review" ${!urgent&&!selectedSlot?'disabled':''}>${urgent?'复核并立即对接':'复核转诊信息'} ${icon('arrow')}</button></div>`;
  setModal(body,'doctor');
}
function validateTransfer(){if(!hasCurrentMatch()||!selectedDoctor||!ranked.some(d=>d.id===selectedDoctor.id)){toast('病例或匹配已变更，请重新匹配后再转诊。');return false;}if(!sourceHospital.trim()||!sourceDoctor.trim()){toast('请补充来源医院和发起医生');return false;}if(!analysis?.valid||!selectedDoctor)return false;if(!analysis.urgent){const slot=buildSchedule(selectedDoctor.id).find(d=>d.date===selectedDate)?.slots.find(s=>s.id===selectedSlot?.id);if(!slot||slot.remaining<=0||selectedDate<localDay()){toast('所选时段已不可用，请重新选择');return false;}}return true;}
function reviewTransfer(){if(!validateTransfer())return;const d=selectedDoctor;setModal(`<div class="modal-title-bar"><small>最后一步 · 医生复核</small><h2>确认转诊信息</h2></div><div class="confirmation"><div class="confirm-banner">${analysis.urgent?'紧急转诊将交由接收中心协调，需接收方确认后衔接后续安排。':'申请将保存至当前工作台，接诊安排需联系院方确认。'}</div><div class="confirm-grid">${[['患者信息',patientLabel(patient)],['接收医院','北京安贞医院安徽医院'],['来源医院',sourceHospital],['发起医生',sourceDoctor],['意向接诊医生',`${d.name} · ${d.department}`],['转诊安排',analysis.urgent?'紧急 · 立即对接':`${formatDate(selectedDate)} ${selectedSlot.session} ${selectedSlot.time}`]].map(([l,v])=>`<div class="confirm-field"><small>${l}</small><strong>${esc(v)}</strong></div>`).join('')}</div><div class="confirm-case">${esc(caseText)}</div><label class="consent"><input type="checkbox" id="reviewed"><span>我已复核病例与转诊意向，知悉本次保存尚未发送院方。</span></label></div><div class="modal-footer"><button class="secondary" data-action="back-detail">返回修改</button><button class="primary" data-action="submit" id="submit-referral" disabled>确认转诊 ${icon('check')}</button></div>`,'confirm');}
...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b||b.disabled)return;
  ...
  if(b.dataset.preset){activePreset=b.dataset.preset;patient={...presets.find(p=>p.id===activePreset)};caseText=patient.text;filter='all';invalidateCase();renderWorkbench();return;}
