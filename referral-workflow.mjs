...
export function reviewReferral(record,{decision,reviewer,note,insurance,at=new Date().toISOString()}) {
  ...
  if(!insurance||['type','settlement','materialStatus'].some(key=>!String(insurance[key]||'').trim()))throw Error('请完成医保与材料核对信息。');
  const approved=decision==='approve';
  return {...record,status:approved?'pending':'returned',insurance,
...
export function resubmitReferral(record,{text,insurance,attachments=record.attachments||[],at=new Date().toISOString()}) {
  ...
  return {...record,status:'review',snapshot:{...record.snapshot,text},insurance,attachments,approval:{status:'pending',reviewer:'',note:''},events:[...record.events,{label:'已补充资料，重新提交医务审核',at}],notifications:[...(record.notifications||[]).map(n=>n.channel==='in_app'&&n.status==='unread'?{...n,status:'read',readAt:at}:n),...referralNotifications(record,'review',at)]};
}
